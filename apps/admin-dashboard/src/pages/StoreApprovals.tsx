...
interface RoleRequest {
    ...
    profiles: {
        full_name: string;
        email: string; // Note: email is not on profile usually but let's see if we can get it or if it's in metadata
        store_name: string;
    };
}
                    ...
                    {requests.map((request) => (
                        <div key={request.id} className="bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col md:flex-row items-center gap-6">
                            <div className="flex-1 space-y-1 text-center md:text-left">
                                <h3 className="text-white font-bold text-lg">{request.profiles.store_name || 'New Store'}</h3>
